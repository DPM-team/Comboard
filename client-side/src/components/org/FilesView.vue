  ...
  <div>
    <div>
      <ul v-for="i in numberRows" :key="i">
        <file-display v-for="j in calculateColumns(i)" :key="j" :name="files[(i - 1) * 4 + j - 1].name" :type="files[(i - 1) * 4 + j - 1].type"></file-display>
      </ul>
    </div>
  </div>
</template>
...
<script>
import FileDisplay from "../dashboard/FileDisplay.vue";
export default {
  components: {
    FileDisplay,
  },
